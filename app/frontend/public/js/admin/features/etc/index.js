...
function cloneTemplateElement(templateId) {
  ...
  return element instanceof HTMLElement ? element.cloneNode(true) : null;
}

function createContentEmptyRowElement() {
  const row = cloneTemplateElement("etc-content-empty-template");
  return row instanceof HTMLTableRowElement ? row : document.createElement("tr");
}

function createContentRowElement(item, index, selectedId) {
  const row = cloneTemplateElement("etc-content-row-template");
  if (!(row instanceof HTMLTableRowElement)) return document.createElement("tr");

  const duration = splitDuration(item.durationSeconds);
  const orderCell = row.querySelector('[data-role="order"]');
  ...
  if (titleInput instanceof HTMLInputElement) titleInput.value = item.title;
  if (typeSelect instanceof HTMLSelectElement) typeSelect.value = item.type;
  if (urlInput instanceof HTMLInputElement) urlInput.value = item.url;
  ...
  if (enabledInput instanceof HTMLInputElement) enabledInput.checked = !!item.enabled;
  if (selectedInput instanceof HTMLInputElement) selectedInput.checked = item.id === selectedId;

  return row;
}

function createContentsFragment(contents, selectedId) {
  const fragment = document.createDocumentFragment();
  if (!contents.length) {
    fragment.append(createContentEmptyRowElement());
    return fragment;
  }

  contents.forEach((item, index) => {
    fragment.append(createContentRowElement(item, index, selectedId));
  });
  return fragment;
}
...
function createPlayerRowElement(teamKey, player, index) {
  const row = cloneTemplateElement("etc-player-row-template");
  if (!(row instanceof HTMLTableRowElement)) return document.createElement("tr");

  const orderCell = row.querySelector('[data-role="order"]');
  const fields = ["name", "position", "mainHero", "tier"];
...
function createPlayerTeamElement(teamKey, players) {
  const shell = cloneTemplateElement("etc-player-team-template");
  if (!(shell instanceof HTMLDivElement)) return document.createElement("div");

  const label = shell.querySelector('[data-role="team-label"]');
  const body = shell.querySelector("tbody");
  if (label) label.textContent = teamKey === "team1" ? "Team 1" : "Team 2";

  if (body instanceof HTMLTableSectionElement) {
    const fragment = document.createDocumentFragment();
    players.forEach((player, index) => {
      fragment.append(createPlayerRowElement(teamKey, player, index));
    });
...
function createPlayersFragment(playersByTeam) {
  const fragment = document.createDocumentFragment();
  fragment.append(createPlayerTeamElement("team1", playersByTeam.team1));
  fragment.append(createPlayerTeamElement("team2", playersByTeam.team2));
  return fragment;
}
...
export function createEtcModule(ctx) {
  ...
  function render() {
    ...
    contentBody.replaceChildren(createContentsFragment(etc.breakContents, etc.selectedBreakContentId));
    shell.replaceChildren(createPlayersFragment(etc.players));
  }
  ...
  function bind() {
    ...
    const playerShell = document.getElementById("etc-player-shell");
    ...
    if (sponsorInput instanceof HTMLInputElement) sponsorInput.addEventListener("input", () => queueRealtimeSync(false));
    if (minutesInput instanceof HTMLInputElement) minutesInput.addEventListener("input", () => queueRealtimeSync(false));
    if (secondsInput instanceof HTMLInputElement) secondsInput.addEventListener("input", () => queueRealtimeSync(false));
    ...
    if (playerShell) {
      playerShell.addEventListener("input", (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement)) return;
        if (!target.dataset.team) return;
        queueRealtimeSync(false);
      });
    }
  }
